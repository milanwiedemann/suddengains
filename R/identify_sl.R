#' @title Identify sudden loss in a wide dataset.
#'
#' @description Function identify sudden losses in a wide dataset. automatically checks whether first session gains are calculated by analysing the variable names looking for session with 0 (intake) before a letter.
#'
#' @param data A dataset in wide format with an id variable and the sudden gains variables.
#' @param cutoff A number specifying the cut-off for criterion 1. Negative value for identifying sudden losses.
#' @param id_var_name A string of the id variable name.
#' @param sl_var_name A string of the sudden gains variable name.
#' @param identify_sl_1to2 Logical value to indicate whether first session gains should be in dataset, works only if s0 variable is provided, default is TRUE


#' @return A wide dataset indicating at which between session interval a sudden gain occured for each person in \code{data}.
#' @export

identify_sl <- function(data, cutoff, id_var_name, sl_var_name, identify_sl_1to2 = FALSE) {
  ...
  data <- data %>%
    ...
    dplyr::select(!!rlang::sym(id_var_name), dplyr::starts_with(sl_var_name))


  # identify_sl <- function(data)
  ...
  for (i in 1:base::nrow(df)) {
    ...
    for (j in 3:(base::ncol(df) - 1)) {
      ...
      crit2[i, j - 2] <- ((df[i, j - 1] - df[i, j]) <= (-1 * (.25 * df[i, j - 1])))
      ...
      if (sum_n_pre == 3 & sum_n_post == 3) {
        crit3[i, j - 2] <-
          mean_pre - mean_post < -1 * (2.776 * base::sqrt(((2 * sd_pre ^ 2) + (2 * sd_post ^ 2)) / (3 + 3 - 2)))
      } else if (sum_n_pre == 2 &
                 sum_n_post == 3) {
        # Adjusting critical value for missing value in pregain mean
        crit3[i, j - 2] <-
          mean_pre - mean_post < -1 * (3.182 * base::sqrt(((2 * sd_pre ^ 2) + (2 * sd_post ^ 2)) / (2 + 3 - 2)))
      } else if (sum_n_pre == 3 &
                 sum_n_post == 2) {
        # Adjusting critical value for one missing value in postgain mean
        crit3[i, j - 2] <-
          mean_pre - mean_post < -1 * (3.182 * base::sqrt(((2 * sd_pre ^ 2) + (2 * sd_post ^ 2))  / (3 + 2 - 2)))
      } else if (sum_n_pre == 2 &
                 sum_n_post == 2) {
        # Adjusting critical value for one missing value in pregain and postgain mean each
        crit3[i, j - 2] <-
          mean_pre - mean_post < -1 * (4.303 * base::sqrt(((2 * sd_pre ^ 2) + (2 * sd_post ^ 2))  / (2 + 2 - 2)))
  ...
  if (sum(str_detect(names(data), "[:alpha:]0")) == 1) {
    ...
    for (i in 1:(base::ncol(df)-3)) {
      sl_col_names[i] <- base::paste("sl_", i, "to", i + 1, sep = "")
    }
  }
  ...
  if (sum(str_detect(names(data), "[:alpha:]0")) == 0) {
    ...
    sl_col_names <- base::c()
    for (i in 1:(base::ncol(df)-3)) {
      sl_col_names[i] <- base::paste("sl_", i + 1, "to", i + 2, sep = "")
    }
  }


  names(crit123) <- sl_col_names
  ...
  if (identify_sl_1to2 == TRUE) {
    data %>%
      dplyr::left_join(data_crit123, by = id_var_name) %>%
      as.tibble()
  } else if (identify_sl_1to2 == FALSE) {
    if (sum(str_detect(names(data), "[:alpha:]0")) == 1) {
      data %>%
        dplyr::left_join(data_crit123, by = id_var_name) %>%
        select(-sl_1to2) %>%
