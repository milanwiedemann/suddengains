...
#'                  id_var_name = "id",
#'                  sg_var_list = c("bdi_s1", "bdi_s2", "bdi_s3", "bdi_s4", "bdi_s5",
#'                                  "bdi_s6", "bdi_s7", "bdi_s8", "bdi_s9", "bdi_s10", "bdi_s11"))
#'
#' @return
#' @export
plot_sg_intervals <- function(data, id_var_name, sg_var_list, identify_sg_1to2 = FALSE) {
    ...
    # Select data
    sgdata_miss <- sgdata %>%
        dplyr::select(id_var_name, all_of(sg_var_list))
    ...
    data_missing <- miss_var_summary(sgdata_miss) %>%
        dplyr::mutate(pct_miss = pct_miss / 100,
               ...
               variable = fct_rev(factor(variable, levels = sg_var_list))) %>%
    ...
    plot_sg_miss_pct <- data_missing %>%
        ...
        ggplot2::ggplot(aes(x = variable, y = value, fill = name)) +
        ...
        ggplot2::theme(legend.key = element_rect(color = NA, fill = NA),
    ...
    count_intervals_data <- suddengains::count_intervals(data = data, id_var_name = id_var_name, sg_var_list = sg_var_list)

    plot_count_intervals_data_temp <- tibble(total = count_intervals_data[[1]],
    ...
    plot_sg_intervals <- plot_count_intervals_data %>%
        ggplot2::ggplot(aes(x = id, y = value, fill = name)) +
        ggplot2::geom_bar(position = "fill", stat = "identity") +
        ggplot2::scale_fill_viridis_d(name = "", direction = -1) +
    ...
    # Plot
    plot_sg_miss_pct / plot_sg_intervals + plot_layout(heights = c(4, 1))
