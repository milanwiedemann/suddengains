...
vignette: >
  %\VignetteIndexEntry{Using the suddengains R package to analyse sudden gains}
...
More about the theoretical background of this package and why it might be helpful can be found in our preprint @R-suddengains.

The following describes the data and the main functions of the package trying to follow the sequence of a regular sudden gains study.
...
<!-- TODO: ADD TABLE WITH MORE DETAILS ABOUT WHAT THE PATTERN IS. -->
...
Each plot will automatically return a warning message about how many missing values were present for each of these five components in the above order.
...
plot_sg_rq <- plot_sg_rq + 
              scale_x_discrete(labels = c("First", "n-2", "n-1", "n",
                                          "n+1", "n+2", "n+3", "Last"))
