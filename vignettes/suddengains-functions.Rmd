...
If multiple sudden gains were experienced by a case, the argument `multiple_sg_select` can be used to specify which gain to select, in the example below the first gain will be slected.
...
Depending on the research questions it might be of interest to select the largest gain, see the example below.
...
Each plot will automatically return a warning message about how many missing values were present for each of these five components mentiond above.
The warning messages from the BDI plot can be interpreted the following way:
