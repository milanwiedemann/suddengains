...
To not use one of the criteria for identifying sudden gains or sudden losses you can switch them off.
The first and second crieria can be set to `NULL`, the third has to be set to `FALSE`.
...
identify_sg(data = sgdata,
            ...
            identify_sg_1to2 = TRUE,
            crit123_details = TRUE) %>% 
    dplyr::select(dplyr::starts_with("sg"))
...
identify_sl(data = sgdata,
            ...
            identify_sg_1to2 = TRUE,
            crit123_details = TRUE) %>% 
    dplyr::select(dplyr::starts_with("sl"))
```
