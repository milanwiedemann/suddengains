...
By default the argument `return_id_lgl` is set to `FALSE`, this simply adds a new variable named `sg_select` at the end of the data frame specified in the `data` argument.
The newly calculated variable `sg_select` is logical and contains information whether a case is selected (`TRUE`) or not selected (`FALSE`) based on the method specified.
...
*Note:* x<sub>1</sub> to x<sub>6</sub> are consecutive data points of the primary outcome measure. "x" = Present data; "." = Missing data. Bold "**N**" represent the pregain session for each "pattern".
...
Some more information about this can be found in the section "Adaptations to the original sudden gains criteria" below.
...
To illustrate this functionality I need to create a data frame with a grouping variable.
...
This function works identical to the plot function for one group.
There are some more arguments to specify the colours to be used for the different groups.
...
The function `plot_sg_trajectories()` can plot individual trajectories of cases.
...
The function can also be combined with a filter function to explore specific sudden gains cases, for example (1) all sudden gain at session 3, or (2) three randomly selected (`select_n = 3`) cases who experienced more than one sudden gain (`dplyr::filter(sg_freq_byperson > 1`).
...
# 2. Create plot including 3 (select_n = 3) cases with 
...
```{r, fig.show = 'hold', fig.width=5.5, fig.height=4}
# 2. Show 3 (select_n = 3) cases with more than 1 gain (dplyr::filter(sg_freq_byperson > 1))
plot_trajectories_3
```
...
- `available_between_sess_intervals_sg`: The total number of between-session intervals that could be analysed for sudden gains.
